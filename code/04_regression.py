...
Int: cap_x_size      = cap_intensity * ln_at

Hypotheses
----------
H1: β(cap_intensity) < 0 and significant
...
print(f"  R&D firms: {(df['cap_intensity']>0).sum():,} "
      f"({(df['cap_intensity']>0).mean()*100:.1f}%)")

# ── Variables ─────────────────────────────────────────────────────────────────
# ADAPT: change these to your own variable names
DV       = "roa"
X_MAIN   = "cap_intensity"
INTERACT = "cap_x_size"       # cap_intensity * ln_at
CONTROLS = ["ln_at", "leverage", "cash_ratio"]
# Build interaction if not already present
if "cap_x_size" not in df.columns:
...
print(f"  β(cap_intensity) = {b_x:.4f}{stars(p_x)} (p={p_x:.3f})")
if p_x < 0.10:
    if b_x < 0:
        ...
        print("  Robustness check: lag cap_intensity by 2-3 years")
    else:
...
# ── H2 Diagnostic ─────────────────────────────────────────────────────────────
print("\n── H2 Diagnostic ────────────────────────────────────")
print(f"  β(cap_x_size) = {b_int:.4f}{stars(p_int)} (p={p_int:.3f})")
if p_int < 0.10:
    if b_int > 0:
